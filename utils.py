...
def get_matrix_size_from_file(file: TextIO) -> Tuple[int, int]:
    x = y = 0
...
def get_matrix_size(file_path: str) -> Tuple[int, int]:
    with open(file_path, "r") as file:
        ret = get_matrix_size_from_file(file)
    return ret


def create_map_from_file(file: TextIO, x: int, y: int) -> np.memmap:
...
def create_map(file_path: str, x: int, y: int) -> np.memmap:
    with open(file_path, "r") as file:
        ret = create_map_from_file(file, x, y)
